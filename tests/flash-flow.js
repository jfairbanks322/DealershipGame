...
(async()=>{const app=createApp({dbPath:':memory:',teacherKey:'test-key'});await new Promise(r=>app.server.listen(0,'127.0.0.1',r));const base=`http://127.0.0.1:${app.server.address().port}`,browser=await chromium.launch({headless:true});try{
 ...
 const t=await h.newPage(),s=await students[0].newPage();for(const p of [t,s])p.on('pageerror',e=>errors.push(e.message));
 ...
 console.log('PASS teacher bank/send, live mobile popup while typing, privacy/auth/room isolation, simultaneous first-three winners, retry safety, input preservation, report credit and reset');
