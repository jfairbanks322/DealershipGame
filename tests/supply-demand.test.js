...
test('separate supply-demand game runs ten rounds without math and keeps math scores separate',async()=>{
 ...
 for(let round=1;round<=10;round++){const i=g.catalog[round-1];g=await req('student',url+'/check',{id:i.id,price:i.expected/100,stock:35});assert.equal(g.check.correct,true);assert.deepEqual(g.player.wrongRounds,[]);assert.ok(!g.player.badges.includes(6));if(round===1){await req('student',url+'/check',{id:'2',price:4.9,stock:35},400);await req('student',url+'/promotion',{id:'bogo',target:'1'},400);}await req('student',url+'/ready',{});g=await req('teacher',url);g=await req('teacher',url+'/run',{version:g.version});if(round<10)g=await req('teacher',url+'/next',{version:g.version});}
 const p=(await req('student',url)).player;assert.equal(p.reports.length,10);assert.ok(p.reports.every(r=>r.penalty===0));assert.equal((await req('student','/leaderboard?lessonId=supply-demand-v1')).board.length,1);assert.equal((await req('student','/leaderboard?lessonId=cost-markup-v1')).board.length,0);
