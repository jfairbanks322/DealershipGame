...
The competitive mode is fully functional for classrooms of up to 30 students with 171 challenges across four mix-and-match round formats. **Story Spark** contains 144 prompts across 16 genres—including poetry, suspense, comedy, fantasy, finish-the-story, and connect-the-start-and-end challenges. **Make It Vivid** asks students to transform a plain description into a sensory scene. **Create the Hero** and **Create the Villain** ask students to build layered characters for a shared scenario. Teachers can switch formats every round without restarting the game or leaderboard. The mode also includes student-selected preset avatars, fixed random teams, server-authoritative writing timers, draft recovery, automatic submission, private moderation, randomized anonymous presentation, one-vote ballots, own-response blocking, tie resolution, team scoring, podium reveals, running writer rankings by total and per-round average podium points, score corrections, final standings, CSV export, and a printable Save-as-PDF report.

The cooperative Story Machine mode removes teams, scores, and voting. The teacher chooses one of six shared story recipes—or lets the game choose randomly—so every writer begins with the same protagonist, setting, central problem, genre, tone, and opening. A second control adds a thematic lens: choose from 100 broad topics organized into ten classroom-friendly groups, draw one randomly, or enter a custom topic. Students have five minutes to complete eight guided sentence stems arranged as a beginning–middle–end arc: character, atmosphere, goal, first escalation, setback, group reaction, decisive choice, and resolution. Every prompt connects its plot beat to the shared topic without requiring students to repeat the topic word. The teacher then uses an animated slot-machine reveal to choose one response for each beat and can re-spin any selection. Locked stems, present-tense guidance, sentence cleanup, a fixed opening, and connective transition lines turn the independently written ideas into one cohesive read-aloud story.
...
`npm test` validates all 171 competitive challenges, the four round formats, the 16 Story Spark categories, and the cooperative story scaffold. `npm run smoke` runs the detailed six-student competitive rehearsal, the complete cooperative Story Machine flow, and a 30-student capacity round. The capacity rehearsal also confirms a full room still permits valid reconnects and rejects a 31st student.
