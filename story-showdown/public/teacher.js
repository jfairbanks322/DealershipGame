(function () {
  ...
  function rosterMarkup(teamSelect = true) {
    ...
    return `<div class="roster-list">${state.players.map((player) => `
      <div class="roster-row">
        <div class="student-name"><i class="connection-dot ${player.connected ? "on" : ""}"></i>${esc(player.name)}</div>
  ...
  function podiumMarkup() {
    ...
    return `<div class="podium-grid">${[1,2,3].flatMap((place) => {
      ...
      return placed.length ? placed.map((result) => `<article class="podium-card ${place === 1 ? "first" : place === 2 ? "second" : "third"}" style="--team-color:${esc(result.teamColor)}"><div class="place">${place}</div><b>${esc(result.label)} · ${esc(result.studentName)}</b><p>${esc(result.teamName)}</p><blockquote>${esc(result.text)}</blockquote><div class="points-pop">+${result.points.toLocaleString()} team points</div></article>`) : [`<article class="podium-card hidden-podium ${place === 1 ? "first" : place === 2 ? "second" : "third"}"><div><div class="place">${place}</div><p>Waiting for reveal…</p></div></article>`];
    }).join("")}</div>`;
  }
