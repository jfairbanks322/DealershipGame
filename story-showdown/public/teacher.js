(function () {
  ...
  const customFormatDefaults = {
    story: { category: "Custom", text: "", criteria: [] },
    vivid: { category: "Description Lab", text: "Rewrite the plain description as a vivid scene. Keep the same basic event, but make the reader experience it.", criteria: ["Sensory detail", "Precise verbs", "Fresh comparison"] },
    hero: { category: "Character Lab — Hero", text: "Create the main character for this scenario. Give them a goal, a fear, a contradiction, and one choice only they would make.", criteria: ["Clear motivation", "Strength and flaw", "Memorable detail"] },
    villain: { category: "Character Lab — Villain", text: "Create the antagonist for this scenario. Give them a believable goal, a persuasive reason, a dangerous flaw, and a line they refuse to cross.", criteria: ["Believable motive", "Threat and humanity", "Distinctive presence"] }
  };
  ...
  function roundFormatFor(id = "story") {
    return state?.roundFormats?.find((item) => item.id === id) || state?.roundFormats?.[0] || {
  ...
  function promptView() {
    ...
    const selectedFormat = prompt?.format || "story";
    const format = roundFormatFor(selectedFormat);
    const selectedCategory = selectedFormat === "story" && state.categories.includes(prompt?.category) ? prompt.category : "all";
    ...
    return `<div class="dashboard-grid"><section class="panel"><div class="panel-head"><div><p class="section-kicker">Round ${state.roundNumber + 1}</p><h2>Prompt lab</h2></div><span class="phase-pill">${state.promptBank.length + state.customPrompts.length} saved prompts</span></div><div class="panel-body prompt-lab">
      <div class="round-format-grid" aria-label="Round format">${state.roundFormats.map((item) => `<button class="round-format-card format-${esc(item.id)} ${selectedFormat === item.id ? "active" : ""}" type="button" data-action="format-prompt" data-format="${esc(item.id)}" aria-label="Choose ${esc(item.label)}"><span>${esc(item.icon)}</span><b>${esc(item.label)}</b><small>${esc(item.summary)}</small></button>`).join("")}</div>
      ${selectedFormat === "story" ? `<div class="category-rail" aria-label="Quick prompt categories">${state.categories.map((category) => `<button class="category-card ${selectedCategory === category ? "active" : ""}" type="button" data-action="category-prompt" data-category="${esc(category)}" aria-label="Preview a ${esc(category)} prompt"><span>${esc(categoryIcon(category))}</span><b>${esc(category)}</b></button>`).join("")}</div>` : `<div class="format-explainer format-${esc(selectedFormat)}"><span>${esc(format.icon)}</span><div><b>${esc(format.label)}</b><p>${esc(format.summary)} Every entry still moves through anonymous presentation, voting, and team scoring.</p></div></div>`}
      <div class="prompt-filters ${selectedFormat === "story" ? "" : "workshop"}">${selectedFormat === "story" ? `<select id="category-filter" aria-label="Prompt category"><option value="all" ${selectedCategory === "all" ? "selected" : ""}>All categories</option>${state.categories.map((category) => `<option ${selectedCategory === category ? "selected" : ""}>${esc(category)}</option>`).join("")}</select>` : ""}<select id="length-filter" aria-label="Response length"><option value="all" ${selectedLength === "all" ? "selected" : ""}>Any length</option><option value="short" ${selectedLength === "short" ? "selected" : ""}>Short</option><option value="medium" ${selectedLength === "medium" ? "selected" : ""}>Medium</option><option value="long" ${selectedLength === "long" ? "selected" : ""}>Long</option></select><button class="button soft" data-action="reroll-prompt" data-format="${esc(selectedFormat)}">Reroll ${selectedFormat === "story" ? "prompt" : "challenge"}</button><button class="button" data-action="blank-prompt" data-format="${esc(selectedFormat)}">Create my own</button></div>
      ${prompt ? `<form id="prompt-form" class="prompt-preview format-${esc(selectedFormat)}"><div class="prompt-title-row"><p class="section-kicker">${isTeacherDraft ? "Create your round challenge" : "Selected challenge"}</p><div class="prompt-badge-row"><span class="round-format-badge"><b>${esc(format.icon)}</b>${esc(format.label)}</span><span class="prompt-category-badge"><b>${esc(categoryIcon(prompt.category))}</b>${esc(prompt.category)}</span></div></div>${isTeacherDraft ? '<p class="muted prompt-help">Edit what students will receive, then choose the guidance and timer.</p>' : ""}<div class="prompt-editor">${selectedFormat !== "story" ? `<div class="field wide challenge-source-field"><label>${esc(format.sourceLabel)}</label><textarea name="source" required minlength="10" maxlength="1200" placeholder="Enter the ${esc(format.sourceLabel.toLowerCase())} students will use…">${esc(prompt.source || "")}</textarea></div>` : ""}<div class="field wide"><label>${selectedFormat === "story" ? "Prompt text" : "Student task"}</label><textarea name="text" required minlength="10" maxlength="1200" placeholder="Write your creative-writing challenge…">${esc(prompt.text)}</textarea></div>${prompt.criteria?.length ? `<div class="field wide"><label>Success targets</label><div class="challenge-criteria">${prompt.criteria.map((criterion) => `<span>✓ ${esc(criterion)}</span>`).join("")}</div></div>` : ""}<div class="field"><label>Category</label><input name="category" maxlength="80" value="${esc(prompt.category)}"></div><div class="field"><label>Difficulty</label><select name="difficulty">${["accessible","intermediate","challenge","teacher choice"].map((item) => `<option ${item === prompt.difficulty ? "selected" : ""}>${item}</option>`).join("")}</select></div><div class="field"><label>Response length</label><select name="responseLength">${["short","medium","long"].map((item) => `<option ${item === prompt.responseLength ? "selected" : ""}>${item}</option>`).join("")}</select></div><div class="field"><label>Timer (seconds)</label><input type="number" name="timerSeconds" min="30" max="900" value="${prompt.timerSeconds || state.settings.defaultDuration}"></div><div class="field wide"><label>Length guidance</label><input name="suggestion" maxlength="80" value="${esc(prompt.suggestion || "Teacher-selected length")}"></div></div><div class="button-row"><button class="button ghost" data-action="save-custom" type="button">Save custom challenge</button><button class="button large" type="submit">Start round ${state.roundNumber + 1}</button></div></form>` : `<div class="empty-state"><div><strong>Ready for a challenge?</strong>Choose a round format, draw from its bank, or build your own.<div class="button-row" style="justify-content:center"><button class="button" data-action="reroll-prompt" data-format="story">Surprise me</button><button class="button ghost" data-action="blank-prompt" data-format="story">Create my own</button></div></div></div>`}
      ${!prompt ? "" : `<p class="fine-print">Edits apply to this round. Use “Save custom challenge” to keep a copy in this game’s library.</p>`}
  ...
  function writingView() {
    ...
    return `<section class="projector-stage challenge-projector format-${esc(prompt.format || "story")}"><div><p class="section-kicker">Round ${state.roundNumber} · ${esc(prompt.formatIcon || categoryIcon(prompt.category))} ${esc(prompt.formatLabel || prompt.category)}</p><div class="timer-display timer-live">${formatTime(remainingSeconds(round))}</div>${prompt.source ? `<div class="projector-source"><span>${esc(prompt.sourceLabel || "Scenario")}</span><strong>${esc(prompt.source)}</strong></div>` : ""}<h2 class="projector-copy">${esc(prompt.text)}</h2>${prompt.criteria?.length ? `<div class="challenge-criteria centered">${prompt.criteria.map((criterion) => `<span>✓ ${esc(criterion)}</span>`).join("")}</div>` : ""}<div class="prompt-meta" style="justify-content:center"><span class="meta-tag">${esc(prompt.suggestion)}</span><span class="meta-tag">${esc(prompt.responseLength)} response</span></div><div class="writing-status">${round.submissionCount} of ${state.playerCount} responses submitted</div><div class="progress-track"><span style="width:${submittedPercent}%"></span></div></div></section>
  ...
  function promptPayload(form) {
    ...
    const format = state.draftPrompt?.format || "story";
    const formatSpec = roundFormatFor(format);
    return {
      id: state.draftPrompt?.id || `teacher-${Date.now()}`,
      format,
      source: data.get("source") || "",
      text: data.get("text"), category: data.get("category"), difficulty: data.get("difficulty"),
      criteria: state.draftPrompt?.criteria || customFormatDefaults[format]?.criteria || [],
      writingLabel: state.draftPrompt?.writingLabel || formatSpec.writingLabel,
      placeholder: state.draftPrompt?.placeholder || formatSpec.placeholder,
  ...
  app.addEventListener("click", async (event) => {
    ...
    try {
      ...
      else if (action === "format-prompt") await teacherAction("teacher:preview-prompt", { filters: { format: button.dataset.format, category: "all", responseLength: "all" } });
      else if (action === "reroll-prompt") await teacherAction("teacher:preview-prompt", { filters: { format: button.dataset.format || state.draftPrompt?.format || "story", category: document.getElementById("category-filter")?.value || "all", responseLength: document.getElementById("length-filter")?.value || "all" } });
      else if (action === "category-prompt") await teacherAction("teacher:preview-prompt", { filters: { format: "story", category: button.dataset.category, responseLength: "all" } });
      else if (action === "blank-prompt") {
        const format = button.dataset.format || state.draftPrompt?.format || "story";
        const formatSpec = roundFormatFor(format);
        const defaults = customFormatDefaults[format] || customFormatDefaults.story;
        await teacherAction("teacher:select-prompt", { prompt: { id: `teacher-${Date.now()}`, format, source: "", text: defaults.text, category: defaults.category, criteria: defaults.criteria, writingLabel: formatSpec.writingLabel, placeholder: formatSpec.placeholder, difficulty: "teacher choice", responseLength: "medium", suggestion: "Teacher-selected length", timerSeconds: state.settings.defaultDuration } });
        requestAnimationFrame(() => document.querySelector(`#prompt-form textarea[name="${format === "story" ? "text" : "source"}"]`)?.focus());
      }
