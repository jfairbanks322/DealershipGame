(function () {
  ...
  function writingView() {
    ...
    return `<div class="student-writing"><div class="student-timer timer-box"><span>${round.pausedRemainingMs !== null ? "Timer paused" : "Time remaining"}</span><strong class="timer-live">${formatTime(remainingSeconds(round))}</strong></div><section class="student-prompt challenge-prompt format-${esc(prompt.format || "story")}"><div class="prompt-title-row"><div class="prompt-badge-row"><span class="round-format-badge"><b>${esc(prompt.formatIcon || categoryIcon(prompt.category))}</b>${esc(prompt.formatLabel || "Story Spark")}</span><span class="prompt-category-badge"><b>${esc(categoryIcon(prompt.category))}</b>${esc(prompt.category)}</span></div><span class="difficulty-badge">${esc(prompt.difficulty)}</span></div>${prompt.source ? `<div class="challenge-source"><span>${esc(prompt.sourceLabel || "Scenario")}</span><strong>${esc(prompt.source)}</strong></div>` : ""}<h2>${esc(prompt.text)}</h2>${prompt.criteria?.length ? `<div class="challenge-criteria">${prompt.criteria.map((criterion) => `<span>✓ ${esc(criterion)}</span>`).join("")}</div>` : ""}<div class="prompt-meta"><span class="meta-tag">${esc(prompt.suggestion)}</span><span class="meta-tag">${esc(prompt.responseLength)} response</span></div></section><section class="writing-desk"><div class="desk-heading"><label class="field-label" for="response-draft">${esc(prompt.writingLabel || "Your response")}</label><span>${prompt.format === "vivid" ? "Make every word earn its place." : prompt.format === "story" ? "Make the room lean in." : "Build someone readers will remember."}</span></div><textarea id="response-draft" maxlength="12000" placeholder="${esc(prompt.placeholder || "Start writing here…")}">${esc(round.myDraft || "")}</textarea><div class="writing-tools"><span id="word-count" class="word-count">${wordCount(round.myDraft)} words</span><button class="button large teal" data-action="submit-response">Submit response</button></div><p class="fine-print">You can edit until you submit. If time runs out, your current draft submits automatically.</p></section></div>`;
  }
