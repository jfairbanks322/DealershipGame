...
function simulate(g, careers) {
  ...
  for (const p of players) {
    ...
    if (p.skippedRound === g.round) {
      p.reports.push({round:g.round, skipped:true, items:[], revenue:0, cost:0, fees:sabotageFees, sabotageFees, penalty:0, profit:-sabotageFees, units:0, promotion:"none"});
      continue;
    }
    ...
    const revenue = items.reduce((a, x) => a + x.revenue, 0),
      ...
      fees = items.reduce((a, x) => a + x.fee, 0) + sabotageFees,
      penalty =
