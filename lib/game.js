...
function pricing(g, p, b) {
  ...
  if (previous === 0) award(p, 6);
  if (p.failedChecks?.[key]) award(p, 10);
  ...
  return { correct: true, message: "Math checked. Your price is saved." };
}
...
function simulate(g, careers) {
  ...
  players.filter((p) => p.skippedRound !== g.round).forEach((p) => evaluate(g, p, players, careers(p.userId)));
