...
function pricing(g, p, b) {
  ...
  if(rulesFor(g).mathChecks!==false)require("./math-progress").record(p,g.round,verdict.correct);
  let penalty = 0;
  if (!verdict.correct) {
    p.failedChecks ??= {};
  ...
  if (verdict.correct && rulesFor(g).mathChecks !== false && previous === 0) award(p, 6);
...
function simulate(g, careers) {
  ...
  for (const p of players) {
    ...
    if (p.skippedRound === g.round) {
      p.reports.push({round:g.round, skipped:true, items:[], revenue:0, cost:0, fees:sabotageFees+mysteryFees+hintFees, hintFees, sabotageFees, mysteryFees, penalty:0, profit:-sabotageFees-mysteryFees-hintFees, units:0, promotion:"none"});
      continue;
    }
    ...
    const revenue = items.reduce((a, x) => a + x.revenue, 0),
      ...
      fees = items.reduce((a, x) => a + x.fee, 0) + sabotageFees + mysteryFees + hintFees,
      penalty =
