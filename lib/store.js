...
function openStore(filename) {
 ...
 CREATE TABLE IF NOT EXISTS careers(game TEXT NOT NULL,userId TEXT NOT NULL REFERENCES users(id),profit INTEGER NOT NULL,win INTEGER NOT NULL,PRIMARY KEY(game,userId));`);
  ...
  for (const [column, fallback] of [["palette", "classic"], ["colorMode", ""]]) {
    if (!db.prepare("PRAGMA table_info(users)").all().some(c => c.name === column))
      db.exec(`ALTER TABLE users ADD COLUMN ${column} TEXT NOT NULL DEFAULT '${fallback}'`);
  }
  return db;
}
