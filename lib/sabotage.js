...
const missions=[
 {id:'coupon',name:'Coupon Caper',icon:'🎟️',success:'Your decoy coupons sent the lunch crowd on a delicious detour.',failure:'The printer stamped your restaurant logo on every decoy coupon. Subtle.'},
 {id:'mascot',name:'Mascot Mix-up',icon:'🦊',success:'A suspiciously enthusiastic mascot redirected the queue.',failure:'Your mascot waved at the security camera and handed out business cards.'},
 {id:'drive',name:'Drive-thru Decoy',icon:'🚗',success:'Your pop-up drive-thru signs persuaded customers to take the scenic route.',failure:'The getaway scooter ran out of battery in front of the whole queue.'}
];
...
function spin(g,p,body,roll=()=>crypto.randomInt(10000)) {
 ...
 const mission=missions.find(m=>m.id===(body.mission||'coupon'));check(mission,'Choose an available sabotage mission.');
 ...
 const guarded=require('./alliances').guard(g,require('./alliances').group(g,target.userId)).active;
 const chance=Math.max(5,tier.chance-attempts.length*15-(guarded?10:0));
 const value=roll(),success=value<chance*100;
 const event={id:crypto.randomUUID(),round:g.round,attacker:p.owner,restaurant:p.restaurant,target:target.restaurant,attackerId:p.userId,targetId:target.userId,mission:mission.id,missionName:mission.name,story:success?mission.success:mission.failure,guarded,attempt:attempts.length+1,cost:tier.cost,chance,roll:value,success,mode:mode(g),backstab,allianceSize,rate:Math.round(rate*100),dueRound:sales?g.round+1:null,damage:success&&!sales?4000:0};
 if(guarded){require('./alliances').consumeGuard(g,target.userId);require('./classroom').event(g,target.owner,'allianceGuard','Team Guard used: 10-point reduction with a 5% minimum chance');}
...
module.exports={missions,tiers,expense,spin,mode,settle,inbox};
