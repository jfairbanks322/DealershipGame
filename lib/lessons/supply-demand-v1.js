...
const competitionLesson = require('./competition');
const events = [
...
function simulateItem(game,p,entry,players){
 const product=game.rules.catalog.find(x=>x.id===entry.id),event=market(game),response=competitionLesson.effects(game,p);
 ...
 const demand=Math.max(0,Math.round(product.popularity*event.demand*Math.exp(-product.sensitivity*(event.sensitivity||1)*(entry.price/product.expected-1))*competition*response.demand/Math.pow(p.menu.length,.38)));
 const units=Math.min(entry.stock,demand),unitCost=Math.round(Math.round(product.cost*event.cost)*response.cost),cost=entry.stock*unitCost,revenue=units*entry.price;
...
module.exports={id:'supply-demand-v1',label:'Supply & Demand',rules,market,checkPricing,simulateItem,readyError:(g,p)=>markup.readyError(g,p)||require("./market-strategy").readyError(g,p)||competitionLesson.readyError(g,p),calculateOffer:markup.calculateOffer,draftFields:[]};
