...
function checkPricing(item, b, rules) {
  ...
  return {
    ...
    message: correct
      ...
      : `Your math needs a correction. Markup amount: $${(amount / 100).toFixed(2)}; selling price: $${(price / 100).toFixed(2)}. The correct price is saved for your chosen markup. You can submit this round.`,
  };
}
function readyError(game, p) {
  if (!p.menu.some((x) => x.addedRound === game.round))
    return "Add this round’s new menu item first.";
  if(require("../markdowns").enabled(game)&&!require("../markdowns").valid(game,p))return "Save this round’s markdown calculation (again if its regular price changed).";
  return null;
}
module.exports = {
  id: "cost-markup-v1",
