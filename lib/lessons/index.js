...
const supply = require("./supply-demand-v1");
const pricing = require("./pricing-v1");
const DEFAULT_LESSON = markup.id;
const lessons = new Map([[markup.id, markup], [supply.id, supply], [pricing.id, pricing]]);
...
function initializeLesson(game, id = DEFAULT_LESSON) {
  ...
  getLesson(id).initialize?.(game);
  return game;
}
function publishedLessons() {
  return [...lessons.values()].map(({ id, label }) => ({ id, label, legacy:id===supply.id }));
}
