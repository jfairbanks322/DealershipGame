...
function view(g,id,host=false,now=Date.now()){
 ...
 return {id:c.id,round:c.round,question:c.question,options:c.options,prizes:c.prizes,startsAt:c.startsAt,endsAt:c.endsAt,serverNow:now,open,eligible:c.eligible.includes(id),winners:c.winners,answered:Object.keys(c.answers).length,participants:c.eligible.length,received:host?Object.keys(c.received||{}).length:undefined,unreceived:host?c.eligible.filter(id=>!c.received?.[id]).map(id=>g.players[id]?.owner||'Student'):undefined,answer:c.answers[id]||null,correct:host||!open?c.correct:undefined,answers:host?Object.entries(c.answers).map(([userId,a])=>({owner:g.players[userId]?.owner,...a})):undefined};
}
