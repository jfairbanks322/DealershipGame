...
function buy(g,p,draw=()=>crypto.randomInt(outcomes.length)) {
 ...
 check(require('./achievements').sum(p)>=price,'You need $5 in earned profit to buy a mystery box.');
 const outcome=outcomes[draw()];
 const result={...outcome,round:g.round,price,net:outcome.amount-price};
 (p.mysteryBoxes??=[]).push(result);return result;
}
module.exports={price,outcomes,expense,buy};
