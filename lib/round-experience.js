...
function receipt(p,r){if(!r)return null;const rows=[],add=(label,amount,timing='Settled with this round')=>{if(amount)rows.push({label,amount,timing});};
 ...
 const accounted=promotions+(r.competitionFee||0)+(boxes.length?boxNet:r.mysteryFees||0)+(r.sabotageFees||0)+(r.hintFees||0)+(r.sabotageStolen||0)-(r.sabotageIncome||0)-(r.flashIncome||0);add('Other recorded fees / credits',-((r.fees||0)-accounted));
