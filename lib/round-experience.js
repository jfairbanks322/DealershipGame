...
function progress(g,p){
 ...
 if(supply)steps.push({id:'strategy',label:g.lessonId==='pricing-v1'?'Save your target market and pricing strategy':'Save your market strategy',done:g.lessonId==='pricing-v1'?!!(p.pricingPlans||[]).find(x=>x.round===g.round)&&!require('./lessons/pricing-v1').planError(g,p):(p.marketStrategies||[]).some(x=>x.round===g.round),required:true});
 ...
 if(require('./markdowns').enabled(g))steps.push({id:'markdown',label:'Calculate and save this round’s markdown',done:require('./markdowns').valid(g,p),required:true});
...
function receipt(p,r){if(!r)return null;const rows=[],add=(label,amount,timing='Settled with this round')=>{if(amount)rows.push({label,amount,timing});};
 ...
 add('Flash Challenge prizes',r.flashIncome||0,'Awarded during planning; already included');
 add('Pricing strategy investment',-(r.strategyFee||0));
 const accounted=promotions+(r.strategyFee||0)+(r.competitionFee||0)+(boxes.length?boxNet:r.mysteryFees||0)+(r.sabotageFees||0)+(r.hintFees||0)+(r.sabotageStolen||0)-(r.sabotageIncome||0)-(r.flashIncome||0);add('Other recorded fees / credits',-((r.fees||0)-accounted));
...
function presets(g){const supply=rulesFor(g).mathChecks===false;return [
 ...
 ...(g.lessonId==='supply-demand-v1'?[{id:'competition',name:'Competition focus',description:'Supply & Demand plus direct/indirect competition challenges. Mystery boxes, sabotage, alliances, and paid hints OFF.',settings:{sabotage:false,boxes:false,alliances:false,hints:false},competition:true}]:[]),
 {id:'full',name:'Full game',description:`Mystery boxes, alliances and next-round sales sabotage ON.${g.lessonId==='supply-demand-v1'?' Competition challenges ON.':supply?' Market research and pricing decisions remain required.':' Paid math hints ON; free guided math keeps its individual settings.'} Core pricing and lesson decisions remain required.`,settings:{sabotage:true,boxes:true,alliances:true,hints:!supply},competition:g.lessonId==='supply-demand-v1'},
 ];}
