...
const names = [
  ...
  ["Special Delivery", "Open your first mystery box."],
  ["Box Office Hit", "Find a reward in a mystery box."],
  ["Receipt of Regret", "Discover a mystery-box mishap."],
  ["Paid for Air", "Open a mystery-box dud."],
  ["Paulie’s Customer", "Buy one of Paulie’s $5 boxes."],
  ["Vinnie Knows You", "Buy any of Vinnie’s mystery boxes."],
  ["Family Business", "Buy from both Paulie and Vinnie in one game."],
  ["Cardboard Collector", "Open three mystery boxes in one game."],
  ["The Full Delivery Experience", "Find a reward, a mishap, and a dud in one game."],
  ["Confidentially Profitable", "Earn a positive net result from Vinnie’s box."],
  ["Kitchen Mischief", "Buy your first sabotage spin."],
  ["Wrench in the Works", "Land a successful sabotage."],
  ["Well, That Spun", "Have a sabotage spin miss."],
  ["Budget Trouble", "Succeed with the $10 sabotage spin."],
  ["Expensive Whoops", "Miss with the $30 sabotage spin."],
  ["Wheel Menu Sampler", "Try all three sabotage prices in one game."],
  ["Frequent Spinner", "Buy sabotage spins in three rounds of one game."],
  ["Triple Trouble", "Land three successful sabotages in one game."],
  ["Not Today, Spatula", "Be targeted by a sabotage that misses."],
  ["Repair Department", "Be targeted by a successful sabotage."],
];
...
function evaluateBonus(p) {
  const boxes = p.mysteryBoxes || [];
  const spins = [...(p.sabotageHistory || [])];
  if (p.sabotageSpin && !spins.some(x => x.id === p.sabotageSpin.id)) spins.push(p.sabotageSpin);
  const incoming = (p.sabotageInbox || []).filter(x=>!x.exposure&&!x.allianceNotice);
  const paulie = boxes.some(x => x.vendor === "Paulie" || (!x.vendor && x.price === 500));
  const vinnie = boxes.some(x => x.vendor === "Vinnie" || x.price === 30000);
  const conditions = [
    boxes.length > 0, boxes.some(x=>x.kind==='reward'), boxes.some(x=>x.kind==='mishap'), boxes.some(x=>x.kind==='nothing'),
    paulie, vinnie, paulie && vinnie, boxes.length >= 3, new Set(boxes.map(x=>x.kind)).size === 3,
    boxes.some(x=>(x.vendor==='Vinnie'||x.price===30000)&&x.net>0),
    spins.length > 0, spins.some(x=>x.success), spins.some(x=>!x.success), spins.some(x=>x.cost===1000&&x.success),
    spins.some(x=>x.cost===3000&&!x.success), [1000,2000,3000].every(cost=>spins.some(x=>x.cost===cost)),
    new Set(spins.map(x=>x.round)).size >= 3, spins.filter(x=>x.success).length >= 3,
    incoming.some(x=>!x.success), incoming.some(x=>x.success)
  ];
  conditions.forEach((yes,i)=>{if(yes) award(p,51+i);});
}
function sum(p) {
  return (p.flashRewards||[]).filter(x=>!p.reports.some(r=>r.round===x.round)).reduce((n,x)=>n+x.amount,0) + p.reports.reduce((a, r) => a + r.profit, 0) - (p.hintRounds || []).filter(round=>!p.reports.some(r=>r.round===round)).length*500 + (p.mysteryBoxes || []).filter(x => !p.reports.some(r => r.round === x.round)).reduce((n, x) => n + x.amount - x.price, 0) - (p.sabotageCosts || []).filter(x => !p.reports.some(r => r.round === x.round)).reduce((a, x) => a + x.amount, 0);
}
module.exports = { badges, award, evaluate, evaluateBonus, sum };
