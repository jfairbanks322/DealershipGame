...
function sum(p) {
  return p.reports.reduce((a, r) => a + r.profit, 0) - (p.hintRounds || []).filter(round=>!p.reports.some(r=>r.round===round)).length*500 + (p.mysteryBoxes || []).filter(x => !p.reports.some(r => r.round === x.round)).reduce((n, x) => n + x.amount - x.price, 0) - (p.sabotageCosts || []).filter(x => !p.reports.some(r => r.round === x.round)).reduce((a, x) => a + x.amount, 0);
}
