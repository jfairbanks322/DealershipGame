...
function guard(g,a){
 const active=a?a.members.filter(id=>g.players[id]&&g.players[id].skippedRound!==g.round):[];
 const rallied=active.filter(id=>g.players[id].guardRallyRound===g.round&&g.players[id].guardRallyGroup===a.id);
 const spent=!!a?.members.some(id=>g.players[id]?.guardUsedRound===g.round);
 return {rallied,required:2,spent,active:enabled(g)&&g.round>=2&&!spent&&rallied.length>=2};
}
function consumeGuard(g,id){const a=group(g,id);if(!guard(g,a).active)return false;for(const member of a.members)g.players[member].guardUsedRound=g.round;return true;}
function name(a){const n=Array.from(a.id).reduce((sum,c)=>sum+c.charCodeAt(0),0);return ['Midnight','Golden','Electric','Secret','Neon','Spicy'][n%6]+' '+['Kitchen Crew','Counter Club','Fry Society','Lunch League','Snack Squad'][Math.floor(n/6)%5];}
function act(g,p,action,b){
 open(g,p);g.alliances??=[];g.allianceInvites??=[];
 if(action==='allianceRally'){
  const mine=group(g,p.userId);check(mine,'Join an alliance first.');check(g.phase==='planning'&&g.round>=2,'Team Guard opens in round 2.');check(!guard(g,mine).spent,'Your alliance already used Team Guard this round.');check(p.guardRallyRound!==g.round,'You already rallied this round.');p.guardRallyRound=g.round;p.guardRallyGroup=mine.id;
 }else if(action==='allianceInvite'){
...
function view(g,id,host){return {enabled:enabled(g),groups:(g.alliances||[]).map(a=>({id:a.id,name:name(a),members:[...a.members],guard:guard(g,a)})),invitations:(g.allianceInvites||[]).filter(i=>host||i.to===id||i.from===id)};}
...
module.exports={guard,consumeGuard,group,act,dissolve,view,saving,enabled};
