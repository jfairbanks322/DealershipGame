...
function notifyFlash(g,force=false){
 ...
 if(old&&old.dataset.challenge!==c.id)old.close();
 ...
 if(!d){
  ...
  d.addEventListener('close',()=>{flashDismissed.add(d.dataset.room+':'+d.dataset.challenge+':'+(flashSnapshot?.open?'open':'closed'));d.remove();});
