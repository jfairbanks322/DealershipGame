function isBusiness() { return game?.lesson?.id === 'pricing-v1'; }
function isSupply() { return ['supply-demand-v1','pricing-v1'].includes(game?.lesson?.id); }
...
function competitionTeacher() {
 if(!isSupply()||isBusiness())return '';const active=game.competition.enabled,locked=!['lobby','results'].includes(game.phase)||game.paused;
...
function marketCard() {
  if(isBusiness())return businessResearch();
  const m=game.market;
...
function strategyPanel() {
  if(isBusiness())return businessStrategy();
...
function strategyRecap(report) {
 if(isBusiness())return businessResults(report);
...
function supplyStudent() {
  ...
  if(game.phase==='lobby') return `${heading()}<div class="welcome"><h2>Your restaurant is ready for business.</h2><p>${isBusiness()?'Pricing & Customers: choose your customers, pricing strategy, menu prices and stock. Discover what works through the results.':'Supply & Demand: choose prices and stock, respond to market changes, and see what customers buy.'} No calculation answers or math penalties.</p><p>Waiting for your teacher to start.</p></div>${leaderboard()}`;
...
function tutorial() {
 ...
 ["Read the research", "Review the round’s event, six customer groups and competitor prices. All restaurants compete for limited customer visits."],
 ["Build your menu", "Add one item each round and choose its price and stock. Every prepared unit costs money, including unsold food."],
 ["Choose your business plan", "Choose a target market and pricing strategy. Bundles and loss leaders require two saved menu items; skimming needs a recent wild creation. A free cost-plus calculator can suggest a price."],
 ["Learn from consequences", "Submit your plan, then review customer groups, satisfaction, sales, waste and profit. There are no required markup answers. Strategy explanations arrive after results."]
 ] : isSupply() ? [
    ...
    ["Choose one menu item", "Pick one of the starting items below. Each round you must add exactly one new item. More choices unlock later, and earlier choices stay available."],
    ...
    ["Build your restaurant", "Your goal is to earn profit over ten rounds. Start by choosing one of the available menu items. Each round you must add exactly one new item; more choices unlock as you play."],
...
function competitionControls() {
 if(!isSupply()||isBusiness())return '';const locked=!['lobby','results'].includes(game.phase)||game.paused;
...
function teacher() {
 return heading()+teacherFocus()+(isBusiness()?businessTeacher():'')+flashTeacher()+fold('teacher-leaderboard','Classroom leaderboard',`<div class="row"><button class="btn ghost small" data-action="export">Download results ↓</button><button class="btn ghost small" data-action="board">Project leaderboard ↗</button></div>${leaderboard()}${roundUpdate()}`);
}
...
root.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if(form.id === 'business-plan-form') {await actionRoom('strategy',b);toast('Business plan saved. Results will show how customers responded.');return;}
...
root.addEventListener("click", async (e) => {
  ...
  try {
    ...
    if (btn.dataset.businessCalculate) {const markup=Number(root.querySelector('#business-markup').value);if(!Number.isFinite(markup)||markup<0||markup>500)throw new Error('Choose a markup from 0 to 500%.');const cost=Number(btn.dataset.businessCalculate),price=Math.round(cost*(1+markup/100));const input=root.querySelector('#pricing-form [name=price]');input.value=(price/100).toFixed(2);input.dispatchEvent(new Event('input',{bubbles:true}));root.querySelector('#business-calculation').textContent=`${cash(cost)} + ${cash(price-cost)} markup = ${cash(price)}. Save below to apply.`;return;}
...
root.addEventListener("change", async (e) => {
 if(e.target.form?.id==='business-plan-form')updateBusinessPlanUI();
