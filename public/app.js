...
function mysteryPanel() {
  ...
  const locked=game.phase!=='planning'||game.round<2||game.paused||game.player.ready||game.player.skippedRound===game.round||current||game.sabotage.balance<500;
  return `<section class="card paulie-panel" id="paulie-panel"><span class="eyebrow">📦 PAULIE’S VERY LEGITIMATE FOOD VENDOR SERVICE</span><h2>“Fell off the back of the truck.”</h2><p>“Five bucks. One box. No questions about the truck.”</p><p class="muted">Optional from round 2, before submitting. One box per round. Each of 30 outcomes is equally likely: 13 rewards, 12 mishaps, 5 duds. Your net result, including the $5 price, stays between −$15 and +$15. Duds still cost $5.</p>${current?`<div class="paulie-reveal" role="status"><span class="eyebrow">${current.kind==='reward'?'A LUCKY FIND':current.kind==='mishap'?'OH, PAULIE…':'WELL, THAT’S A BOX'}</span><h3>${esc(current.title)}</h3><p>${esc(current.description)}</p><strong>Net profit change: ${current.net>=0?'+':''}${cash(current.net)}</strong><p class="tiny">Box: ${cash(current.price)} · ${current.amount>=0?'Reward':'Extra expense'}: ${cash(Math.abs(current.amount))}. Already included in your total profit.</p></div>`:''}<button class="btn orange" data-action="mystery-box" ${locked?'disabled':''}>${current?'Box opened this round':'Buy Paulie’s box · $5'}</button>${game.round<2?'<p class="tiny">Paulie’s truck arrives in round 2.</p>':!current&&game.sabotage.balance<500?'<p class="tiny">You need $5 in earned profit to buy a box.</p>':''}${boxes.length?`<details><summary>Your delivery history (${boxes.length})</summary>${boxes.map(x=>`<p><strong>Round ${x.round}: ${esc(x.title)}</strong> · ${x.net>=0?'+':''}${cash(x.net)}<br>${esc(x.description)}</p>`).join('')}</details>`:''}</section>`;
}
