...
function sectionNavigation() {
 ...
 : game.phase==='planning'
 ? [['#round-checklist','Round overview'],['.catalog','Menu & prices'],['#round-review','Review & submit'],...(!isSupply()?[['fold:math-help','Math help']]:[]),['fold:optional','Optional moves']]
 : game.phase==='lobby'?[]:[['.result-overview','Round results'],['fold:results','Item details'],['fold:rank-update','Leaderboard'],['fold:optional','Optional moves']];
...
function studentExperience() {
 const body=student().replace(heading(),'');
...
root.addEventListener("click", async (e) => {
  ...
  if(btn.dataset.section) {
    ...
    if(target){if(target.tagName==='DETAILS'){target.open=true;openPanels.set(target.dataset.fold,true);}target.setAttribute('tabindex','-1');target.focus({preventScroll:true});target.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'start'});}
    return;
  }
  try {
    if(btn.dataset.nextStep){
      const step=btn.dataset.nextStep==='auto'?game.roundProgress.next:btn.dataset.nextStep;
      const selector={clues:isSupply()?'#round-clues':'#round-goal',challenge:'.competition-panel',strategy:'.strategy-panel',item:'.catalog',review:'#round-review',submit:'#round-review'}[step];
