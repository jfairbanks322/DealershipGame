...
function mysteryPanel() {
  ...
  return `<section class="card paulie-panel" id="paulie-panel"><span class="eyebrow">📦 ${vinnie?'VINNIE’S SUPER SECRET BOX':'PAULIE’S VERY LEGITIMATE FOOD VENDOR SERVICE'}</span><h2>${vinnie?'“My cousin Vinnie knows a guy.”':'“Fell off the back of the truck.”'}</h2><p>${vinnie?'Paulie whispers: “Three hundred bucks. Super secret. Even the box signed a confidentiality agreement.”':'“Five bucks. One box. No questions about the truck.”'}</p><p class="muted">Optional, before submitting. One box per round. Each of 30 outcomes is equally likely: 13 rewards, 12 mishaps, 5 duds. ${vinnie?'Pay $300 up front. Rewards earn $50–$150 net; mishaps lose $70–$150 net after a partial refund. Duds lose the full $300. These figures include the purchase price.':'From round 2: your net result, including the $5 price, stays between −$15 and +$15. Duds still cost $5. Vinnie’s $300 super secret box arrives in round 6.'}</p>${current?`<div class="paulie-reveal" role="status"><span class="eyebrow">${current.kind==='reward'?'A LUCKY FIND':current.kind==='mishap'?'OH, '+esc(current.vendor||'Paulie').toUpperCase()+'…':'WELL, THAT’S A BOX'}</span><h3>${esc(current.title)}</h3><p>${esc(current.description)}</p><strong>Net profit change: ${current.net>=0?'+':''}${cash(current.net)}</strong><p class="tiny">Box: ${cash(current.price)} · ${current.kind==='mishap'&&current.amount>0?'Partial refund':current.amount>=0?'Payout':'Extra expense'}: ${cash(Math.abs(current.amount))}. Already included in your total profit.</p></div>`:''}<button class="btn orange" data-action="mystery-box" ${locked?'disabled':''}>${current?'Box opened this round':`Buy ${vendor}’s ${vinnie?'super secret ':''}box · ${cash(price)}`}</button>${!game.bonusSettings.boxes?'<p class="tiny">Mystery boxes are disabled by your teacher.</p>':''}${game.round<2?'<p class="tiny">Paulie’s truck arrives in round 2.</p>':!current&&game.sabotage.balance<price?`<p class="tiny">You need ${cash(price)} in earned profit to buy this box.</p>`:''}${boxes.length?`<details><summary>Your delivery history (${boxes.length})</summary>${boxes.map(x=>`<p><strong>Round ${x.round}: ${esc(x.title)}</strong> · ${x.net>=0?'+':''}${cash(x.net)}<br>${esc(x.description)}</p>`).join('')}</details>`:''}</section>`;
}
...
function sabotagePanel() {
  ...
  const locked=p.ready||game.paused||p.skippedRound===game.round||!game.sabotage.canSpin||!game.bonusSettings.sabotage;
  return `<section class="card sabotage-panel"><span class="eyebrow">BONUS · SABOTAGE WHEEL</span><h2>A little kitchen rivalry</h2><p>Spend earned profit for a chance to give an opponent a $40 equipment-repair expense. The spin costs money even if it fails. A success unlocks another attempt, up to 3 per round. Each follow-up loses 15 percentage points of success chance. A miss ends your turns and reveals you to everyone you targeted this round. Each restaurant can be targeted once per round.</p><p>Attempt ${Math.min(3,game.sabotage.attempts+1)} of 3 · Available profit: <strong>${cash(game.sabotage.balance)}</strong></p>${log}${p.sabotageSpin?.round===game.round?`<p class="notice">Your spin against ${esc(p.sabotageSpin.target)} ${p.sabotageSpin.success?'worked':'missed'}. Paid ${cash(p.sabotageSpin.cost)}.</p>`:''}<form id="sabotage-form" class="stack"><label>Target restaurant<select name="target" required ${locked?'disabled':''}><option value="">Choose an opponent</option>${targets.map(x=>`<option value="${esc(x.userId)}">${esc(x.restaurant)} · ${esc(x.owner)}</option>`).join('')}</select></label><label>Spin price and success chance<select name="cost" ${locked?'disabled':''}>${game.sabotage.tiers.map(t=>`<option value="${t.cost}" ${game.sabotage.balance<t.cost?'disabled':''}>${cash(t.cost)} · ${t.chance}% success</option>`).join('')}</select></label><button class="btn orange" ${locked||!targets.length||game.sabotage.balance<1000?'disabled':''}>Pay & spin</button></form><p class="tiny muted">${!game.bonusSettings.sabotage?'Sabotage is disabled by your teacher. ':''}Submit locks your spin choice. Already-targeted restaurants are protected until the next round.</p></section>`;
}
function sabotageDialog(event, spinning=false) {
  ...
  dialog.innerHTML=`<h2>${spinning?'Sabotage spin':'Your restaurant was targeted!'}</h2>${spinning?`<div class="wheel-wrap"><span class="wheel-pointer">▼</span><div class="sabotage-wheel" style="background:conic-gradient(#238363 0 ${event.chance}%,#da583b ${event.chance}% 100%)"><span>↗</span></div></div><p>Green = success (${event.chance}%) · orange = miss</p>`:''}<p class="spin-result" role="status">${spinning?'Spinning…':`${event.revealed||!event.success?esc(event.attacker)+' ('+esc(event.restaurant)+')':'An anonymous rival'} targeted you in round ${event.round}. ${event.success?'It worked: a $40 equipment-repair expense was added.':'It missed: no repair expense for you.'}${event.exposedByFailure?' A failed attempt exposed this attacker to everyone they targeted this round.':''}`}</p><button class="btn" ${spinning?'disabled':''}>Got it</button>`;
