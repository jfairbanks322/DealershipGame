...
### Alliance Team Guard and sabotage missions

Alliances now have an automatic crew name, an avatar roster, and live rally progress. From round 2, two non-skipped members can rally before submitting to arm a free Team Guard. It reduces the next incoming sabotage's success chance by 10 percentage points (5% minimum), then is consumed for the entire alliance whether the attack wins or misses. It applies equally to outside attacks and backstabs. Each owner can contribute once per round; changing alliances cannot restore a used guard. Disabling alliances suspends guards along with savings.

Sabotage offers Coupon Caper, Mascot Mix-up, and Drive-thru Decoy story themes with identical costs and economics. A three-step attempt display and live target preview show current success/miss odds, costs, alliance betrayal stakes, and next-round payouts. The server rechecks the guard at spin time. Existing 10% rival and 15–25% ally theft rates, anonymity/exposure rules, three-attempt limit, and teacher repair/sales modes remain in effect.
...
Flash Challenge delivery checks all rooms the signed-in student belongs to, including from My businesses after a reload. It does not depend on an opened restaurant or a pricing request finishing. The teacher sees **Popup received** plus students whose browsers have not acknowledged delivery. Students must refresh once after this update to load the new delivery code; being signed out, offline, or on an old page prevents receipt. Returning to the tab or reconnecting triggers an immediate check.

### Pricing & Customers (new strategy lesson)
**Pricing & Customers** replaces Supply & Demand in the new-room menu. It adds eight pricing strategies, six customer markets, saved randomized events, finite customer competition, free cost-plus assistance, three optional delayed-feedback scenarios, and round/final business reports. No markup answers are required. Existing Supply & Demand rooms retain their original rules; Cost & Markup is unchanged. See [simulation rules and accounting](docs/pricing-simulation.md).
